...
import type { PaiementRecu } from '~/types/tresor'

definePageMeta({ requiresAuth: true })

const { paiementsRecus, institutions } = useMockData()

const statutFilter = ref('')
const institutionFilter = ref('')

const institutionOptions = computed(() => {
  const names = [...new Set(paiementsRecus.value.map(p => p.institution))]
...
const totalRecus = computed(() => filteredData.value.reduce((sum, p) => sum + p.montant, 0))

const columns: ColumnDef<PaiementRecu>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
  ...
  },
  {
    accessorKey: 'payeur',
    header: 'Payeur',
    cell: ({ row }) => h('span', { class: 'text-xs font-medium' }, row.getValue('payeur')),
  },
  {
    accessorKey: 'pspPayeur',
    header: 'PSP',
    cell: ({ row }) => h('span', { class: 'text-xs text-muted-foreground' }, row.getValue('pspPayeur')),
  },
  {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold">Paiements recus</h1>
        <p class="text-sm text-muted-foreground mt-1">Historique des paiements recus par la plateforme</p>
      </div>
    </div>
