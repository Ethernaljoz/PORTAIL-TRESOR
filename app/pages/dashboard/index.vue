...
const { institutions, enrolements } = useInstitutionsData()
const { transactions } = useTransactionsData()
const { paiementsRecus, paiementsEnvoyes, demandesPaiement } = useTransactionsData()
...
const statCards = computed(() => {
  ...
  return [
    ...
    { label: 'Taux de succes', value: `${successRate}%`, icon: 'i-lucide-check-circle', color: 'text-emerald-600 bg-emerald-50' },
...
function typeColor(type: string): string {
  const map: Record<string, string> = { 'Recette': 'text-emerald-600 bg-emerald-50', 'Dépense': 'text-amber-600 bg-amber-50' }
  return map[type] || 'text-muted-foreground bg-muted'
}

function statutColor(statut: string): string {
  const map: Record<string, string> = {
    'Réussi': 'text-emerald-600 bg-emerald-50 ring-emerald-200', 'En attente': 'text-amber-600 bg-amber-50 ring-amber-200',
    'Échoué': 'text-destructive bg-destructive/10 ring-destructive/20', 'Annulé': 'text-muted-foreground bg-muted ring-border',
  }
  return map[statut] || 'text-muted-foreground bg-muted'
}

function notifIcon(type: string): string {
  const map: Record<string, string> = { info: 'i-lucide-info', success: 'i-lucide-check-circle', warning: 'i-lucide-alert-triangle', error: 'i-lucide-x-circle' }
  return map[type] || 'i-lucide-bell'
}

function notifColor(type: string): string {
  const map: Record<string, string> = { info: 'text-blue-600 bg-blue-50', success: 'text-emerald-600 bg-emerald-50', warning: 'text-amber-600 bg-amber-50', error: 'text-destructive bg-destructive/10' }
  return map[type] || 'text-muted-foreground bg-muted'
}
</script>
...
<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1">Vue d'ensemble de la plateforme Tresor</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card class="lg:col-span-1">
        <CardHeader><CardTitle class="text-base">Resume</CardTitle></CardHeader>
        <CardContent class="space-y-4">
          <div class="flex items-center justify-between py-2 border-b border-border">
            <span class="text-sm text-muted-foreground">Paiements recus</span>
            <span class="text-sm font-semibold">{{ paiementsRecus.length }}</span>
          </div>
          <div class="flex items-center justify-between py-2 border-b border-border">
            <span class="text-sm text-muted-foreground">Paiements envoyes</span>
      ...
      <Card class="lg:col-span-2">
        <CardHeader class="flex flex-row items-center justify-between">
          <div>
            <CardTitle class="text-base">Transactions recentes</CardTitle>
            <CardDescription>Dernieres {{ recentTransactions.length }} transactions</CardDescription>
          </div>
          <NuxtLink to="/dashboard/transactions" class="text-xs text-primary hover:underline">Voir tout</NuxtLink>
        </CardHeader>
        <CardContent>
          <div class="space-y-3">
            <div v-for="tx in recentTransactions" :key="tx.id" class="flex items-center gap-3 py-2 border-b border-border last:border-0">
              <span :class="['inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold', typeColor(tx.type)]">{{ tx.type }}</span>
              ...
              <span :class="['inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ring-inset', statutColor(tx.statut)]">{{ tx.statut }}</span>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <Card>
      <CardHeader class="flex flex-row items-center justify-between">
        <div>
          <CardTitle class="text-base">Notifications recentes</CardTitle>
          <CardDescription>{{ unreadNotifs }} non lue(s)</CardDescription>
        </div>
        <NuxtLink to="/dashboard/notification" class="text-xs text-primary hover:underline">Tout voir</NuxtLink>
      </CardHeader>
      <CardContent>
        <div class="space-y-3">
