...
import PraticienForm from '@/components/form/PraticienForm.vue'
...
const { can, isAdmin, isPermissionsReady } = usePermissions()
const { fetcher, deletePraticien } = usePraticiens()

const openForm = ref(false)
const selectedPraticien = ref<Praticien | null>(null)
const deleteTarget = ref<Praticien | null>(null)
const showDelete = ref(false)
const deleting = ref(false)

const hasAccess = computed(() => can('praticiens', 'list') || can('praticiens', 'create') || can('praticiens', 'update') || can('praticiens', 'delete'))
...
function openAdd() {
  selectedPraticien.value = null
  openForm.value = true
}

function openEdit(p: Praticien) {
  selectedPraticien.value = p
  openForm.value = true
}

function confirmDelete(p: Praticien) {
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deletePraticien(deleteTarget.value.id)
    toast('Praticien supprimé', { description: 'Le praticien a été supprimé avec succès.' })
    showDelete.value = false
    deleteTarget.value = null
  }
  catch {
    toast.error('Erreur', { description: 'Impossible de supprimer ce praticien.' })
  }
  finally {
...
const columns: ColumnDef<Praticien>[] = [
  {
    accessorKey: 'name',
    header: 'Nom & Prénoms',
      ...
      <div v-if="hasAccess" class="space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <Button v-if="can('praticiens', 'create')" class="btn-primary h-10 text-[13px]" @click="openAdd">
              <Plus class="size-4" />
              Ajouter
            </Button>
          </div>
        ...
        <!-- Praticien Form Dialog -->
        <PraticienForm v-model:open="openForm" :praticien="selectedPraticien" />
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Voulez-vous vraiment supprimer le praticien <strong>{{ deleteTarget?.name }}</strong> ? Cette action est irréversible.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
